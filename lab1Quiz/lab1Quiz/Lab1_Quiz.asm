...
MAIN:
		...
		ldi counter, 0x01 ;Offset by 1 in order to function properly
LOOP:
...
END_TABLE:
		ldi r16, 0x5F	;Load register with determined end value
		st Y+, r16			;Store endValue at Pointer Y
		rjmp BACK			;Jump to Done

BACK: 
		dec counter		;dec counter
		breq DONE		;Branch to done
		pop r16			;Pop off the stack
		st Y+, r16		;Store the value and increment the pointer
		rjmp BACK		;Jump to End